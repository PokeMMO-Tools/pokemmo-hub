...
const PokemonSearch = ({ sprites }) => {
    ...
    const [showFinalEvolutionsOnly, setShowFinalEvolutionsOnly] = useState(false);
    ...
    const abilityOptionsAlternative = useMemo(() => {
        return abilities
            .map(a => ({
                value: a.en_name || a.name,
                label: a.en_name || a.name
            }))
            .sort((a, b) => a.label.localeCompare(b.label));
    }, []);
    ...
    const handleResetFilters = () => {
        ...
        setShowFinalEvolutionsOnly(false);
    };
    ...
    const filteredPokemon = useMemo(() => {
        return pokemon
            ...
            .filter(mon => {
                ...
                const monEvolutions = mon.evolutions ?? [];

                const hasAbility = selectedAbility ? (() => {
                    // Normalize both selected ability and pokemon abilities for comparison
                    const normalizeAbility = (ability) =>
                        ability.toLowerCase().replace(/\s+/g, '').replace(/[^a-z0-9]/g, '');

                    const normalizedSelected = normalizeAbility(selectedAbility);
                    const normalizedMonAbilities = monAbilityNames.map(normalizeAbility);

                    return normalizedMonAbilities.includes(normalizedSelected);
                })() : true;
                ...
                const isFinalEvolution = showFinalEvolutionsOnly ? monEvolutions.length === 0 : true;

                return hasAbility && hasMoves && hasTypes && hasEggGroup && matchesTier && isFinalEvolution;
            })
            .sort((a, b) => {
                const sumStats = stats => Object.values(stats ?? {}).reduce((sum, val) => sum + val, 0);
                return sumStats(b.stats) - sumStats(a.stats);
            });
    }, [selectedAbility, selectedMoves, selectedTypes, selectedEggGroups, selectedTier, allowOverrideLastAbility, showFinalEvolutionsOnly]);
    ...
    const headerText = useMemo(() => {
        ...
        const finalEvoText = showFinalEvolutionsOnly ? 'Final Evolutions Only' : '';

        const filters = [
            typesText ? `Type: ${typesText}` : '',
            abilityText ? `Ability: ${abilityText}` : '',
            eggGroupText ? `Egg Group: ${eggGroupText}` : '',
            selectedMovesText ? `Moves: ${selectedMovesText}` : '',
            finalEvoText
        ].filter(Boolean);

        return `Pokémon that match: ${filters.join(', ')}`;
    }, [selectedAbility, selectedMoves, selectedTypes, selectedEggGroups, showFinalEvolutionsOnly]);
                ...
                <section>
                    <div className="form-check">
                        <input
                            type="checkbox"
                            className="form-check-input"
                            id="finalEvolutionToggle"
                            checked={showFinalEvolutionsOnly}
                            onChange={() => setShowFinalEvolutionsOnly(!showFinalEvolutionsOnly)}
                        />
                        <label className="form-check-label" htmlFor="finalEvolutionToggle">
